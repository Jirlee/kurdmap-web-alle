...
| 33 | **`aria-sort` روی ستون‌های مرتب شده** | جداول ادمین فقط businesses sort دارد — نیاز به `aria-sort="ascending/descending"` | 🟡 P2 | ❌ |
| 34 | **`aria-current="page"` در navigation** | sidebar و bottom-nav باید صفحه فعلی را مشخص کنند | 🟡 P2 | ❌ |
| 35 | **`aria-expanded` روی sidebar toggle** | دکمه hamburger باید `aria-expanded="true/false"` داشته باشد | 🟡 P2 | ❌ |
...
| 42 | **Fulltext search با highlighting** | جستجوی متنی با برجسته‌سازی کلمات یافت شده | 🟢 P3 | ❌ |
